...
function [power1stHarmonic, power2ndHarmonic] = Image2Power2D(ImageIn, frequencies, max2min, interpolate)
%Determines the Fourier power of a numerosity display using a 2D procedure.
%Suitable when items are circles.
%Canvas must be square.
%A major part of this is finding the edge of the first harmonic, which is
%an obvious local minimum. But it may not be the first local minimum, due
%to pixilation. So we may smooth the series or limited the height of the
%minumum. 

if ~exist('interpolate', 'var') || isempty(interpolate)
    interpolate=0;
end
if ~exist('max2min', 'var') || isempty(interpolate)
    max2min=3;
end

%Examples of this variable 'fourierImg' are shown in Figure 3a,
%Supplementary Figure 7a, 8a and 9a
fourierImg     = abs(single(fftshift(fft2(ImageIn))));

%Sum over all directions for each frequency.
%Examples of this variable 'ring' are shown in Figure 3b, Supplementary
%Figure 7b, 8b and 9b
ring=zeros(1, size(ImageIn, 2)/2);
for ecc=1:length(ring)
    ring(ecc)=sum(fourierImg(frequencies==ecc));
end
                    
%From here, we need to find where the minimum is, to which the first
%harmonic should be summed. There are different ways to do this, some
%faster or slower, some more or less affected by pixilation.

%First, smooth to minimize the effect of pixilation
if interpolate
    interp=(ring+[ring(end-1:end) ring(1:end-2)]+[ring(end) ring(1:end-1)]+[ring(2:end) ring(1)]+[ring(3:end) ring(1:2)])/5; %#ok<NASGU>
else
    interp=ring; %#ok<NASGU>
end
% %Find an edge of the first harmonic which is beyond the maximum power, low,
% %and where the smoothed power distibution moves from a decrease to
% %increase.
[maxVal, maxPos]=max(ring);
%tmp=find((diff([interp 0])>0 & ring<maxVal/max2min));
tmp=find((islocalmin(ring) & ring<maxVal/max2min));
whichMin=tmp(find(tmp>maxPos, 1));

%Sum power to this frequency
power1stHarmonic=sum(ring(1:whichMin));
...
%This may be beyond the Nyquist limit, in which case nothing is
%returned
try
    power2ndHarmonic=sum(ring(whichMin+1:whichMin2));
catch
    power2ndHarmonic=nan;
end
